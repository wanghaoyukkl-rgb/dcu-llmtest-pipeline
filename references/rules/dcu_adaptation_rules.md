...
- 自动生成或改写脚本只允许三类适配：添加/设置 `HIP_VISIBLE_DEVICES`、把模型路径替换为目标节点绝对模型路径、以及为避免端口冲突或支持同节点并发而新增/修改服务监听端口。模型路径优先从 `/public/opendas/DL_DATA/llm-models/` 查找，允许该路径是软链接并解析到 `/public4/...`。其它 dtype、TP/PP/DP、量化、`-cc`/编译配置、上下文长度、显存比例、调度参数均必须按来源原样保留。
- 自动生成或改写脚本不得加入或保留 `rm`、`rm -rf`、`rmdir` 等清理命令；来源文档包含这些命令时按规则省略并记录。
