...
不要在 cookbook 已明确覆盖且卡型匹配时，擅自用本地补充文档覆盖 cookbook 的最佳实践。本地测试指导是补充来源，不是主来源。
...
生成脚本时，将通用测试模式命令和目标卡型小节补充参数合并。文档中出现 `<HOST_IP>`、`<LOCAL_PATH>`、`[internal-link-removed]` 时，不要原样写入最终脚本；应替换为当前计划表分配的 IP/端口/容器路径，或按 blocked 说明处理。
...
- 不删除 DCU、NUMA、通信、量化、MoE、PD/IFB 调度相关环境变量。
- 默认 IFB，只有用户明确要求时才使用 PD 分离模式。
- 模型路径统一替换为容器内 `/model/<模型名>`，不要沿用文档里的 `<LOCAL_PATH>` 或历史目录。
- `host_ip`、`master_ip`、`NODE2_IP`、`--dist-init-addr` 必须按当前任务环境重新生成，不要沿用文档占位。
- 端口必须由计划表统一分配，不能沿用文档中已有端口。
