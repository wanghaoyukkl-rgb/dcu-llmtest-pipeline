...
# 优先读取评测日志判断进度；可选扫描 prediction 文件，若连续 3 条样本疑似乱码，
# 则中断当前评测/服务进程以释放加速卡资源，但保留容器。
#
# 用法（宿主机执行）：
#   nohup bash watch_accuracy.sh <log_file> <status_file> [prediction_path] [container_name] [check_interval] &
#
# 参数：
...
with open(file_path, "r", encoding="utf-8", errors="replace") as fh:
    for line in fh:
        ...
        if garbled(text):
            ...
            consecutive = 0
        if consecutive >= 3:
            print(f"GARBLED:{file_path}:连续 3 条 prediction 疑似乱码")
            sys.exit(10)
        if checked >= 20:
            break

if checked >= 3:
    print(f"OK:{file_path}:已检查 {checked} 条 prediction")
else:
...
while true; do
    ...
    if [ -n "${PREDICTION_PATH}" ] && [ "${PREDICTION_CHECKED}" -eq 0 ]; then
        if in_container; then
        ...
        if [ "${PRED_CODE}" -eq 10 ]; then
            release_accelerator_resources
            write_status "aborted" "prediction 连续 3 条疑似乱码，已中断当前评测并尝试释放加速卡资源；容器保留。" "${PRED_CHECK}"
