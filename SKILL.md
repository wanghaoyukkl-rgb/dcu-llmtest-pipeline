...
当前版本：**v0.6.1-alpha**。版本特性和边界见 `references/current_version.md`，仅在用户询问版本能力或维护 skill 时读取。
...
7. 汇总结果：短任务读取 watcher/orchestrator 的 JSON 状态并报告；多模型或长任务必须由 orchestrator 写 `state.json/events.log/reports/`，Agent 被唤醒后补发总结。
...
- 精度测试启动前必须检查评测工具环境。OpenCompass 正式评测需确认 `opencompass`、`openai`、`math_verify`、`latex2sympy2_extended`、`antlr4`、`human_eval`。
...
- OpenCompass 不要额外生成 `<model>.eval.log` 这类外层评测日志；进度和错误优先使用其输出目录下的 `logs/infer/`、`logs/eval/`、`summary/`，后台编排只写 `orchestrator.log/events.log/state.json`。
- 多模型、长时间 OpenCompass 或存在后续排队任务时，不能依赖 Agent 会话里的临时监控脚本；必须使用 orchestrator 推进、写状态并在终态释放资源。
...
- 状态更新优先读小型 JSON：`/tmp/llm_status.json`、`/tmp/eval_status.json`、`state.json`；后台事件读 `events.log`。不要依赖无事件输出的临时 `monitor_status.log`。
